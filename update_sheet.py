...
<div class="toolbar">
 ...
 <span class="sep"></span>
 <button id="fit">Pattern Zoom</button><button id="all">All Data</button><button id="reset">Reset Zoom</button>
 <span class="sep"></span>
...
function render(timeframe){{
 ...
 // Smart initial zoom around the detected scanner pattern.
 const bars = DATA.frames[tf] || [];
 const ds = DATA.drawings[tf] || [];
 let anchorTimes = [];
 ds.forEach(d => (d.points || []).forEach(p => {{ if (p.time) anchorTimes.push(p.time); }}));

 let fromIndex = Math.max(0, bars.length - (tf === 'Daily' ? 120 : tf === 'Weekly' ? 90 : 60));
 let toIndex = Math.max(0, bars.length - 1);

 if (anchorTimes.length && bars.length) {{
   const idxs = anchorTimes.map(t => bars.findIndex(b => b.time === t)).filter(i => i >= 0);
   if (idxs.length) {{
     const first = Math.min(...idxs);
     const last = Math.max(...idxs);
     const span = Math.max(last - first, 12);
     const pad = Math.max(8, Math.round(span * 0.30));
     fromIndex = Math.max(0, first - pad);
     toIndex = Math.min(bars.length - 1, Math.max(last + pad, bars.length - 1));
     if (toIndex - fromIndex > 180) fromIndex = Math.max(0, toIndex - 180);
   }}
 }}

 priceChart.timeScale().setVisibleLogicalRange({{from: fromIndex - 0.5, to: toIndex + 6}});
 rsiChart.timeScale().setVisibleLogicalRange({{from: fromIndex - 0.5, to: toIndex + 6}});
...
document.getElementById('fit').onclick=()=>render(tf);
document.getElementById('all').onclick=()=>{{priceChart.timeScale().fitContent();rsiChart.timeScale().fitContent();}};
document.getElementById('reset').onclick=()=>render(tf);
document.getElementById('price').ondblclick=()=>render(tf);
document.getElementById('rsi').ondblclick=()=>render(tf);
...
render(tf);
    ...
    CHART_OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    (CHART_OUTPUT_DIR / filename).write_text(page, encoding="utf-8")
    return pattern_chart_url(filename) + "?v=5"
