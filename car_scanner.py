...
def replace_tab(book, name: str, rows: list[list]) -> None:
    try:
        ...
        tab = book.add_worksheet(title=name, rows=max(201, len(rows) + 10), cols=9)
    tab.clear()
    tab.resize(rows=max(2, len(rows)), cols=9)
    tab.update(range_name=f"A1:F{len(rows)}", values=rows, value_input_option="USER_ENTERED")
    tab.freeze(rows=1)
    tab.format("A1:F1", {
    ...
    if len(rows) > 1:
        ...
        tab.format(f"E2:E{len(rows)}", {"numberFormat": {"type": "NUMBER", "pattern": "#,##0.00"}})
        tab.format(f"F2:F{len(rows)}", {"textFormat": {"foregroundColor": {"red": 0.08, "green": 0.37, "blue": 0.80}, "bold": True}})
        green = {"red": 0.84, "green": 0.96, "blue": 0.86}
        if name == BUY_TAB:
            tab.format(f"A2:F{len(rows)}", {"backgroundColor": green})
        else:
            requests = [{"repeatCell": {
                "range": {"sheetId": tab.id, "startRowIndex": i, "endRowIndex": i + 1,
                          "startColumnIndex": 0, "endColumnIndex": 6},
                "cell": {"userEnteredFormat": {"backgroundColor": green}},
                "fields": "userEnteredFormat.backgroundColor",
            }} for i, row in enumerate(rows[1:], start=1) if row[2] == "Buy/Average Out"]
            if requests:
                book.batch_update({"requests": requests})
    widths = [170, 115, 160, 115, 105, 130]
    book.batch_update({"requests": [{"updateDimensionProperties": {
        "range": {"sheetId": tab.id, "dimension": "COLUMNS", "startIndex": i, "endIndex": i + 1},
...
def main() -> None:
    ...
    for stock in stocks:
        ...
        try:
            ...
            cmp_price = round(float(frame["Close"].dropna().iloc[-1]), 2) if not frame.empty else ""
        except Exception as exc:
            print(f"{code}: fetch failed: {exc}")
            rating, high_day = "DATA UNAVAILABLE", ""
            cmp_price = ""
        chart_url = "https://www.tradingview.com/chart/?symbol=" + quote("NSE:" + code, safe="")
    ...
    buy_tab.update(range_name="H1:I2", values=[["Stocks scanned", "CAR passed"], [len(stocks), len(buy_rows) - 1]])
    buy_tab.format("H1:I1", {"textFormat": {"bold": True}, "backgroundColor": {"red": 0.91, "green": 0.95, "blue": 1.0}})
